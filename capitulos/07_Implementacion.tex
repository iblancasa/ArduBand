...
\node[draw] at (-1,3) {LLegada};
\end{tikzpicture}


\caption{Representación temporal de la llegada de paquetes (X está en segundos)} \label{fig:graficabroadcast}
\end{figure}
...
\begin{lstlisting}[style=codigoC]
  void setup(){
      xbee = iniciarSerialXBee();
      bt = iniciarSerialBluetooth();
  }

  xbeeDir[]={addr1,addr2,addr3};
  pack[]={};
    ...
    void loop(){
      if(leer){
        if(bt.data()){
          tempo = bt.getdata();
          leer = false;
          createPack(pack,tempo);
          disableBT();
          au1 = tempo/ 60;
          periodo = (1000 / au1) * 4

          timer(500);
          timer.setFunction(Envio);
        }
      }
    }

    void createPack(pack,tempo){
      for(i = 0 ; i < xbeeDir.size(); i++){
        pack[i] = createPackete(tempo,xbeeDir[i]);
      }
    }
  }
\end{lstlisting}
...
\begin{enumerate}
  \item Se inicializan los pines de los puertos serial.
  ...
  \item En el bucle que se ejecuta continuamente, si leer(*) es verdadero:
    \begin{itemize}
      ...
      \item Se desactiva el Bluetooth (para ahorrar energía)
