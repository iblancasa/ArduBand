...
\begin{itemize}
  ...
  \item Alcance de red de hasta 400ft (120m)
  \item Antena incorporada
  \item Certificado por FCC
  \item 6 pines de entrada para ADC de 10 bit
  \item 8 pines digitales de entrada/salida
  \item Encriptación de 128-bit
  \item Modo de funcionamiento AT o API
\end{itemize}
...
\begin{description}
  \item[AT] \hfill \\
    ...
    que se haya configurado sea la de \text{broadcast}). Esto quiere decir que, en caso que
...
Este problema podríamos achacarlo a la capa \textit{firmware} de la mota (que podría contener mecanismos
...
Como lo que vamos a medir son pulsos por segundo, la notación de los compases es solo didáctica, es decir
...
\end{figure}
  ...
  \begin{tikzpicture}
  \draw[latex-latex, thin, draw=gray] (-1,0)--(16,0) node [right] {$x$};
  \draw[latex-latex, thin, draw=gray] (0,-1)--(0,4) node [above] {$y$};
...
Debido a los continuos problemas por parte del SDK Android \cite{googledevelopers} para conseguir conectar
...
Tras esto, el usuario deberá escribir con números el \textit{tempo} que desea en el campo
...
De la misma forma, se ha trabajado en la interfaz para dispositivos Android Wear.
