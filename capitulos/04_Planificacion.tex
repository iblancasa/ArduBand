...
Como sistema de control de versiones, se utilizará Git. Para guardar los datos,
se utilizará el servicio GitHub.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item{Cálculo de los valores necesarios para mantener el \textit{tempo} en un actor}
    \item{Envío del \textit{tempo} desde el director a los músicos}
    \item{Posibilidad de cambiar el \text{tempo} (por parte del director)}
    \item{Aplicaciones propias para cambiar el \text{tempo}}
...
Para el desarrollo del proyecto se ha contado con solo una persona, el propio autor de este trabajo.\\

Teniendo en cuenta que para el proyecto se han utilizado alrededor de 4 meses, estaríamos hablando
de una necesidad de ``1 persona/mes para un período 4 meses"\ o (si estimamos que el tiempo
empleado en el proyecto se puede reducir de forma lineal) ``4 personas/mes"\ para 1 mes.
...
sistema de comunicación \textit{wireless} algunas bibliotecas desarrolladas por la comunidad-).\\
...
Como se acaba de comentar, se utilizará la plataforma Arduino. Además de comprar
...
La única incógnita que podría plantearse viene por parte de la empresa Digi al utilizar
...
Al comprar en grandes cantidades, probablemente, el precio de algunos de estos
...
Como se ha referido anteriormente en la sección dedicada a hablar de los recursos humanos disponibles,
...
\title{Otros gastos}
Sabiendo que se va a desarrollar una aplicación ``Android"\,
será necesario pagar la cuota para obtener cuenta de desarrollo (25USD, unos 22,5\euro) \cite{desarrollaAndroid}.\\
...
menor será el costo (producción en masa). Es decir, la inversión se verá ``más repartida". También habrá que tener en
...
\begin{gather*}
  1.426 + (51 \times \textup{Directores} +46.5 \times \textup{Músicos}) = \textup{Gasto material} \\ \\
  \textup{Gasto material} - ((\textup{Beneficio/dispositivo} \times \textup{Total dispositivos}) +\\
  (51 \times \textup{Directores} + 46.5 \times \textup{Músicos})) = \textup{Coste final}
\end{gather*}
...
\[
  1.426 + (51 \times 10 + 46.5 \times 99 \times 10) = 47.971  \textup{\euro}
\]
\[
  47.971 - (((10 \times 100) \times 10 + 46550) = -8574  \textup{\euro}
\]
...
(aún así, en el segundo caso, probablemente, se seguirían obteniendo beneficios ya que un mismo trabajador
